...
client.on("guildMemberAdd", member => {
    ...
    if (guvenilirlik === false) {
        ...
        hgkanal.send(`
<a:hac:833259957130100737> Sunucumuza hoş geldin ${member} - (\`${member.id}\`)

<a:hac:833259957130100737> Hesabın \`(${kuruluş})\` tarihinde oluşturulmuş
    
<a:hac:833259957130100737> Seninle beraber \`${guild.memberCount}\` kişi olduk! Tagımızı <a:tag:833306511333130260> alarak bizlere destek olabilirsin.
    
<a:hac:833259957130100737> Sunucu kurallarımız <#${config.kurallar}> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.
`)
    } else {
        member.roles.set([config.Suspecious])
        member.setNickname(`Yeni Hesap`)
        hgkanal.send(`
        ${member} Adlı kullanıcı sunucumuza katıldı hesabı şüpheli olduğu için şüpheli hesap rolünü verdim.
        `)
    }
});