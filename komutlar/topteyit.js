...
if(!message.member.roles.cache.has(config.kayıtcıRolü) && !message.member.hasPermission(8)) return;
    ...
    let topteyit = xd.filter(dat => message.guild.members.cache.has(dat)).sort((a, b) => Number((data[b].erkek || 0) + (data[b].kadın || 0)) - Number((data[a].erkek || 0) + (data[a].kadın || 0))).map((value, index) => `\`${index + 1}.\` ${message.guild.members.cache.get(value)} Toplam **${((data[value].erkek || 0) + (data[value].kadın || 0))}** (**${((data[value].erkek || 0))}** Erkek **${((data[value].kadın || 0))}** Kadın)`).splice(0, 25);
