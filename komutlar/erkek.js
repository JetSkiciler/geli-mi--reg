...
exports.run = async(client, message, args) => {
    ...
    if (data) {
        ...
        \`vip için ${config.prefix}vip @etiket/id\``))
    }
    if (config.erkekRolleri.some(e => uye.roles.cache.has(e))) return message.channel.send(embed.setDescription(`Bu Kullanıcı Zaten Kayıtlı.`)).then(m => m.delete({ timeout: 7000 }))
    ...
    if (!x) {
        message.channel.send(embed.setDescription(`
        ${uye} kişisinin ismi başarıyla \`${name}\` olarak değiştirildi, ${message.guild.roles.cache.get(config.erkek1)} rolü verildi.
   ...
   if (uye.roles.cache.has(config.vipRolü) ? await uye.roles.set(config.erkekRolleri) && uye.roles.add(config.vipRolü) : uye.roles.set(config.erkekRolleri))
    ...
    await db.push(`isimler.${uye.id}`, { isim: name, rol: config.erkek1, yetkili: message.author.id, zaman: Date.now() })
